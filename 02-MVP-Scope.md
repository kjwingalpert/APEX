...
**Deliverables in v1 (all three core capabilities):**
...
| Stance Tracker | Visual Pro/Con breakdown of where key government entities/policymakers stand on an issue, built from analyzed statements/actions |
| Action Tracer | Basic timeline of historical precedent and regulatory evolution for an issue |
...
- **Multi-tenant hardening / isolation guarantees:** single effective tenant during MVP, but schema is tenant-scoped anyway.
- **Scale engineering:** no horizontal scaling, caching layers, CDN strategy, load balancing until usage demands it.
...
- Cost ceiling for AI usage (`[CONFIRM]`) is respected; alerts when approaching it.

## Definition of "scope done"

- Testers can complete lobbying/testimony/coalition prep tasks using all three capabilities.
